function __print_projectblaze_functions_help() {
cat <<EOF
Additional ProjectBlaze functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- blazegerrit:   A Git wrapper that fetches/pushes patch from/to ProjectBlaze Gerrit Review.
- blazerebase:   Rebase a Gerrit change and push it again.
- blazeremote:   Add git remote for ProjectBlaze Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for ProjectBlaze Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the ProjectBlaze model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch blaze_$target-$variant
        fi
    fi
    return $?
}
...
function blazeremote()
{
    ...
    then
        ...
        local PFX="ProjectBlaze/"
    else
        local PROJECT=$REMOTE
    fi

    local BLAZE_USER=$(git config --get review.review.projectblaze.org.username)
    if [ -z "$BLAZE_USER" ]
    then
        git remote add blaze ssh://review.blazeos.org:29418/$PFX$PROJECT
    else
        git remote add blaze ssh://$BLAZE_USER@review.projectblaze.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/ProjectBlaze/$PROJECT
...
function blazegerrit() {
    ...
    local user=`git config --get review.review.projectblaze.org.username`
...
function blazerebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "ProjectBlaze Gerrit Rebase Usage: "
