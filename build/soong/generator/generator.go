...
func init() {
	android.RegisterModuleType("blaze_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = blazeExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = blazeExpandVariables(ctx, dep_file)
	...
	cmd := blazeExpandVariables(ctx, String(g.properties.Cmd))
